...
- **L15. Les plugins ne se COUPLENT JAMAIS entre eux.** Un plugin NE DOIT JAMAIS importer le code ou les internes d'un autre plugin. Toute interaction inter-plugins passe par l'**Event Bus typé**. Un plugin PEUT déclarer une **dépendance d'ordre/capacité** (résolue par le Plugin Manager, [ch.10 §10.5](./docs/10-plugins.md)), mais cela ne l'autorise pas à dépendre de l'implémentation de l'autre. *(P4/P5, ch.10)*
